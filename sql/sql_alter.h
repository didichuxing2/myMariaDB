...
public:
  ...
  */
  ...
  enum operations_used_flags
  {
    ...
    ALTER_RECREATE              = 1L << 11,
    // Set for ADD PARTITION
    ALTER_ADD_PARTITION         = 1L << 12,
    // Set for DROP PARTITION
    ALTER_DROP_PARTITION        = 1L << 13,
    // Set for COALESCE PARTITION
    ALTER_COALESCE_PARTITION    = 1L << 14,
    // Set for REORGANIZE PARTITION ... INTO
    ALTER_REORGANIZE_PARTITION  = 1L << 15,
    // Set for partition_options
    ALTER_PARTITION             = 1L << 16,
    // Set for LOAD INDEX INTO CACHE ... PARTITION
    // Set for CACHE INDEX ... PARTITION
    ALTER_ADMIN_PARTITION       = 1L << 17,
    // Set for REORGANIZE PARTITION
    ALTER_TABLE_REORG           = 1L << 18,
    // Set for REBUILD PARTITION
    ALTER_REBUILD_PARTITION     = 1L << 19,
    // Set for partitioning operations specifying ALL keyword
    ALTER_ALL_PARTITION         = 1L << 20,
    // Set for REMOVE PARTITIONING
    ALTER_REMOVE_PARTITIONING   = 1L << 21,
    // Set for ADD FOREIGN KEY
    ADD_FOREIGN_KEY             = 1L << 22,
    // Set for DROP FOREIGN KEY
    DROP_FOREIGN_KEY            = 1L << 23,
    // Set for EXCHANGE PARITION
    ALTER_EXCHANGE_PARTITION    = 1L << 24,
    // Set by Sql_cmd_alter_table_truncate_partition::execute()
    ALTER_TRUNCATE_PARTITION    = 1L << 25,
    // Set for ADD [COLUMN] FIRST | AFTER
    ALTER_COLUMN_ORDER          = 1L << 26,
